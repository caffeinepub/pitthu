...
import { generateBookingId, saveBooking } from "../lib/bookingStorage";

type RideType = "private" | "shared" | "bus" | "bike" | "scooty";
...
export default function BookRidePage() {
  ...
  const doBooking = async () => {
    ...
    try {
      ...
      success();
      toast.success(
        `Booking confirmed! ₹${finalFare} | Earned ${coins} Pitthu Coins 🪙`,
      );
      // Try ICP as bonus (non-blocking)
      createBooking({
    ...
    <div className="min-h-screen bg-muted py-12">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <form onSubmit={handleBookNow}>
          {sameLocation && (
            <div
              className="mb-4 rounded-xl border border-red-300 bg-red-50 dark:bg-red-900/20 dark:border-red-500/40 px-4 py-3 text-sm text-red-600 dark:text-red-400 font-medium"
                      ...
                      {(["normal", "rain", "night"] as WeatherMode[]).map(
                        (w) => (
                          <button
                            ...
                            data-ocid="ride.toggle"
                          >
                            {w === "normal" && <Sun className="w-3.5 h-3.5" />}
                            {w === "rain" && (
            ...
            <div className="lg:col-span-1">
              <Card className="shadow-card sticky top-20">
                ...
                <CardContent className="space-y-3">
                  ...
                  {/* Fare Breakdown */}
                  <div className="bg-blue-950/60 backdrop-blur-md border border-blue-500/30 rounded-2xl p-4 space-y-2">
                  ...
                  <div className="rounded-xl border border-border p-3 bg-muted/50">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={() => {
                          tap();
                          toast.error("Emergency services alerted! Stay safe.");
                        }}
          ...
          {from && to && from !== to && (
            <div className="mt-6">
              <GeminiRouteHappenings from={from} to={to} />
            </div>
          )}
        </form>
      </div>
